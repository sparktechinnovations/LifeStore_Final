...
const TABLE_HEAD = [
  ...
  { id: 'agent.name', label: 'Agent Name', align:"center"},
  { id: 'agent.agentUid', label: 'Agent ID', align:"center" },
                    ...
                    agentState?.payments?.map((agents, index) => {
                      ...
                      return (
                        <TableRow hover key={_id} tabIndex={-1} role="checkbox">
                          <TableCell align="center" component={Link} to={`/dashboard/agentProfile/${agent._id}`}>{serialNumber}</TableCell>
                          <TableCell scope="row" padding="none" align="center" component={Link} to={`/dashboard/agentProfile/${agent._id}`}>
                            {`${agent?.name}  ${agent?.deleteStatus === 'true' ? '(Deleted)' : ''}`}
                          </TableCell>
                          <TableCell align="center" component={Link} to={`/dashboard/agentProfile/${agent._id}`}>{agent?.agentUid}</TableCell>
                          <TableCell align="center" component={Link} to={`/dashboard/agentProfile/${agent._id}`}>{amount.toFixed(2)}</TableCell>
                          <TableCell align="center" component={Link} to={`/dashboard/agentProfile/${agent._id}`}>
                            {date ? new Date(date).toLocaleDateString('en-GB') : 'N/A'}
                          </TableCell>
                          <TableCell align="center" component={Link} to={`/dashboard/agentProfile/${agent._id}`}>{new Date(invoiceFrom).toLocaleDateString('en-GB')}</TableCell>
                          <TableCell align="center" component={Link} to={`/dashboard/agentProfile/${agent._id}`}>{new Date(invoiceTo).toLocaleDateString('en-GB')}</TableCell>
                        </TableRow>
                      );
                    })
                  )}
