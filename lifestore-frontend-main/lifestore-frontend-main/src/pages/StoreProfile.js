...
import { getStore, getStoreCommission, postStorePayments } from '../features/storeSlice';
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(postStorePayments(newPaymentRecord))
      .then((result) => {
        ...
        handleShowNotification(`Amount ${paymentData.amount} received successfully`, 'success');
       dispatch(getStore({ id }));
       dispatch(getStoreCommission({ id, params: searchParams }));
       handleCloseDialog();
       setPaymentData({ amount: '', date: '' });
            ...
            >
              <CardContent>
                ...
                <Box display="flex" justifyContent="center" marginBottom="16px" className="avatar-container">
                  <AvatarUploader profilePic={store?.store?.profilePic?.url} />
                </Box>
      ...
      <Container maxWidth="xl">
        <Grid container spacing={3} style={{ display: 'flex', justifyContent: 'center' }}>
          <Grid item xs={12} sm={6} md={3}>
            <AppWidgetSummary title="Pending Commission" total={+pendingCommisiion?.toFixed(2) || 0} color="info" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <AppWidgetSummary
              title="Credits"
          ...
          <Stack alignItems={'end'} m={3}>
            <Stack direction={{ xs: 'column', md: 'row' }} gap={3} alignItems={'end'} width={'100%'}>
              ...
              <Box width={'100%'}>
                <Button
                  variant="container"
                  style={{ backgroundColor: 'black', color: 'white', marginRight: '10px', height: '55px' }}
