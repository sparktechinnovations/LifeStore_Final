...
export const getAgentCommission = createAsyncThunk(
  ...
  async ({ id, params }, { rejectWithValue, dispatch, getState, fulfillWithValue }) => {
    ...
    try {
      const { auth } = getState();
      const response = await axios.get(`/api/commission/agent-commissions/${id}?${params}`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${auth.token}`,
        },
...
state.paymentAmount =''
  ...
  extraReducers: {
    ...
    [agentList.fulfilled]: (state, action) => {
      ...
      state.totalCommissions = action?.payload?.totalCommission.commissions;
    },
...
export const { resetEditAgent,resetAmount } = agentSlice.actions;
