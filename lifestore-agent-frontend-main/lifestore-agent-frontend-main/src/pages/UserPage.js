...
import { customerList } from '../features/customerSlice';
...
export default function UserPage() {
  ...
  const handleClickRow = (customerId) => {
    navigate(`/dashboard/customerProfile/${customerId}`);
  };
  ...
  return (
    <>
      ...
      <Container>
        ...
        >
          <>
            <Stack direction={{ xs: 'column', md: 'row' }} gap={3} alignItems={'end'} width={'100%'} p={3}>
              <Box width={'100%'}>
                <StyledSearch
                  fullWidth
            ...
            <Stack justifyContent="center" display={'flex'} alignItems={'center'} mb={3} width={'100%'}>
                ...
                <TableBody style={{ backgroundColor: 'white', cursor: 'pointer' }}>
                  {customerState?.customerListLoader ? (
                    ...
                    customerState?.customers?.map((customer, index) => {
                      const { _id, name, phoneNumber, email, totalSpending, createdAt } = customer;
                      ...
                      return (
                        <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedcustomer}>
                          ...
                          <TableCell component="th" scope="row" padding="none" onClick={() => handleClickRow(_id)}>
                          ...
                          <TableCell align="left" onClick={() => handleClickRow(_id)}>
                            {phoneNumber}
                          </TableCell>
                          <TableCell align="left" onClick={() => handleClickRow(_id)}>
                            {email || 'N/A'}
