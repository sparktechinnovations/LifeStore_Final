...
const initialState = {
  ...
  restoreProductError: '',
  getProductNamesError: '',
  getProductNamesLoader: false,
  getProductNamesSuccess: false,
};
    ...
    try {
      ...
      if (error.code === 'ERR_NETWORK') {
        <Alert severity="error">
          Network Error! Try Refreshing page! If problem persists, please Contact Support!
        </Alert>;
...
export const productSlice = createSlice({
  ...
  extraReducers: {
    ...
    [getProductNames.pending]: (state) => {
      state.products = [];
      state.getProductNamesLoader = true;
      state.getProductNamesSuccess = false;
      state.getProductNamesError = '';
      state.pages = 1;
      state.page = 1;
      state.documentCount = 0;
      state.limit = 10;
    ...
    [getProductNames.fulfilled]: (state, action) => {
      
      state.products = action?.payload?.productList;
      state.getProductNamesLoader = false;
      state.getProductNamesSuccess = true;
      state.getProductNamesError = '';
      state.documentCount = action?.payload?.documentCount;
      state.page = action?.payload?.page;
      state.limit = action?.payload?.limit;
    ...
    [getProductNames.rejected]: (state, action) => {
      state.products = [];
      state.getProductNamesLoader = false;
      state.getProductNamesSuccess = false;
      state.getProductNamesError = action.payload?.message || 'Failed to get agent names';
      state.pages = 1;
      state.page = 1;
      state.documentCount = 0;
      state.limit = 10;
