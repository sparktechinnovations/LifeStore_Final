...
export const generateInvoice = createAsyncThunk(
  ...
  async (obj, { rejectWithValue, dispatch, getState, fulfillWithValue }) => {
    try {
      ...
      const response = await axios.post(`/api/invoice`, {...obj}, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${auth.token}`,
        },
...
export const getCustomer = createAsyncThunk(
  'customer/get',
  async ({ phone }, { rejectWithValue, dispatch, getState, fulfillWithValue }) => {
    try {

      const { auth } = getState();
      const response = await axios.get(`/api/invoice/customer/${phone}`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${auth.token}`,
        },
